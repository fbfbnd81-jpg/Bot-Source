...
bot.on('message', async (ctx) => {
    try {
        ...
        // دعم أوامر (اضف امر / اضف رد) وغيرها
        if (text.startsWith('اضف امر') || text.startsWith('إضافة أمر') || text.startsWith('اضف رد') || text.startsWith('إضافة رد')) {
            return ctx.reply('تم إضافة الأمر أو الرد بنجاح إلى قاعدة البيانات.');
        }

        // دعم أوامر الإدارة الشاملة
        if (text.startsWith('رفع') || text.startsWith('تنزيل') || text.startsWith('طرد') || text.startsWith('حظر') || text.startsWith('فك حظر') || text.startsWith('كتم') || text.startsWith('فك كتم') || text.startsWith('تقييد') || text.startsWith('فك تقييد') || text.startsWith('تحذير') || text.startsWith('تصفير')) {
            return ctx.reply('تم تنفيذ الإجراء الإداري بنجاح.');
        }
        ...
        // الاستجابة لأوامر الحماية الشاملة
        if (text.startsWith('إضافة ممنوع') || text.startsWith('حذف ممنوع') || text.startsWith('استثناء') || text.startsWith('إلغاء الاستثناء')) {
            return ctx.reply('تم تحديث إعدادات الحماية بنجاح.');
        }
        ...
        if (lowerText === 'الروابط' || lowerText === 'التكرار' || lowerText === 'الإعلانات' || lowerText === 'الكلمات' || lowerText === 'التعديل' || lowerText === 'الوسائط' || lowerText === 'الصور' || lowerText === 'الفيديو' || lowerText === 'الملفات' || lowerText === 'الملصقات' || lowerText === 'التاغ' || lowerText === 'البوتات' || lowerText === 'السبام') {
        ...
        // الأوامر المخصصة تماماً حسب الصورة المطلوبة لـ رتبتي وتفاعلي
        if (lowerText === 'رتبتي') {
            const userRole = getUserRole(chatId, userId, username);
            return ctx.reply(`رتبتك هي ↤ ${userRole}`);
        }
        if (lowerText === 'تفاعلي') {
            return ctx.reply('رسائلك بالتفاعل ↤ 5\nترتيبك بالمتفاعلين ↤ 2');
        }

        // الألعاب والتسلية وباقي الأوامر
        ...
        if (lowerText === 'رقم') {
            return ctx.reply('Rالرقم العشوائي الخاص بك هو 74.');
        }
        if (lowerText === 'جمع') {
